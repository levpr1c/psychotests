...
| ❤️ **Heart** | Cardiovascular tendencies (IBC/PPS/DE/AG/NCD/ZM) | 25 × Likert 1-5 |
...
- 👥 **Multi-user** profiles — each user has their own test history
- 📈 **Statistics screen** with bar charts, per-test filtering, and colored result list
- 🎨 **Rich-styled UI** — result panels with colored borders, hover effects, aligned columns
- 🔀 **Shuffled questions** — same-scale items spread apart for better test quality
- 🎲 **Randomized Luscher** — colors appear in random order each session, each round
- 💾 **SQLite** storage — persisted test history, no data loss
- 🏗️ **Frozen binary** — single-file executable via PyInstaller (auto-build on tag)
